...
namespace RealEstate_Dapper_Api.Repositories.ServiceRepository
{
    public class ServiceRepository : IServiceRepository
    {
        ...
        {
            string query = "Insert into Service(ServiceName,ServicesStatus) values (@serviceName,@servicesStatus)";
            var parameters = new DynamicParameters();
            parameters.Add("@serviceName", createServiceDto.ServiceName);
            parameters.Add("@servicesStatus", createServiceDto.ServicesStatus);
        ...
        public async Task<List<ResultServiceDto>> GetAllServiceAsync()
        {
            ...
            {
                var values = await conntection.QueryAsync<ResultServiceDto>(query);
        ...
        {
            string query = "Update Service Set ServiceName=@serviceName,ServicesStatus=@servicesStatus where ServiceID=@serviceID";
            var parameters = new DynamicParameters();
            parameters.Add("@serviceID", updateServiceDto.ServiceID);
            parameters.Add("@serviceName", updateServiceDto.ServiceName);
            parameters.Add("@servicesStatus", updateServiceDto.ServicesStatus);
            using (var connection = _context.CreateConnection())
            {
                await connection.ExecuteAsync(query, parameters);
            }
